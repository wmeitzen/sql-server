...
-- Drop if exists
IF OBJECT_ID('dbo.StatsOptimize', 'P') IS NOT NULL
	DROP PROCEDURE dbo.StatsOptimize;
GO

CREATE PROCEDURE dbo.StatsOptimize
	-- Command dispatch
	@Command VARCHAR(32) = 'OPTIMIZE'           -- OPTIMIZE, SHOWUSAGE, STATSOPTIMIZEDATA_DDL (null/invalid prints help)
	-- Target selection
	...
	,@with_sample_percent NVARCHAR(10) = 'TF2371' -- 'TF2371' = size-adaptive curve (TF2371-style, not the trace flag's algorithm), a numeric percent (forced), or NULL (engine default)
	...
	,@HotSkipHours INT = 24                      -- busiest stats: eligible again after this many hours
	,@ColdSkipHours INT = 720                    -- idle stats: eligible again after this many hours (720 = 30 days)
	,@DefaultSkipHours INT = 168                 -- used when usage is untrusted or @UsageModel = 'None' (168 = 1 week)
	-- Priority weighting
	,@UsagePriorityWeight DECIMAL(9, 2) = 5.0    -- weight applied to the 0-100 usage percentile in the priority score
	-- Stats row-count bounds
	,@MinNumberOfStatsRows BIGINT = 1000         -- skip statistics with fewer rows than this
	,@MaxNumberOfStatsRows BIGINT = null         -- skip statistics with more rows than this (null = no upper bound)
	-- Usage trust gate
	,@MinInstanceUptimeDays INT = 14
	-- Column (auto-created) statistics handling
	,@AutoStatsMode VARCHAR(20) = 'ParentUsage' -- ParentUsage, Modification
	-- Throttle and behavior
	,@MaxDOP INT = null                           -- NULL = server default; 1 = serial; 2+ = limit parallelism per stat update
	,@TimeLimit INT = null                       -- seconds; stop starting new updates past this
	,@Execute CHAR(1) = 'N'                       -- N previews, Y runs
	,@LogToTable CHAR(1) = 'Y'                    -- log to CommandLog table
	,@CommandLog NVARCHAR(512) = 'master.dbo.CommandLog' -- 3-part name of the CommandLog table
	,@StatsOptimizeTableMode VARCHAR(20) = NULL  -- NULL/None = no state; StatsOptimizeData = dedicated table; CommandLog = legacy snapshot rows
	,@StatsOptimizeTable NVARCHAR(512) = 'master.dbo.StatsOptimizeData' -- 3-part name of the state table (used when mode = StatsOptimizeData)
AS
BEGIN
	SET NOCOUNT ON;
	...
	-- When in doubt, in the unlikely scenario of this procedure deadlocking with
	-- a business transaction, this procedure should be the victim
	...
	BEGIN
		...
		PRINT '-- Target: ' + @StatsOptimizeTable;
		PRINT '';
	...
	BEGIN
		...
		PRINT 'EXAMPLES';
		PRINT '=================================================================';
		PRINT '  -- Preview what would be updated across all user databases';
		PRINT '  EXEC master.dbo.StatsOptimize';
		PRINT '    @Command = ''OPTIMIZE'', @Databases = ''USER_DATABASES'';';
		PRINT '';
		PRINT '  -- Show the usage distribution (read-only)';
		PRINT '  EXEC master.dbo.StatsOptimize';
		PRINT '    @Command = ''SHOWUSAGE'', @Databases = ''SalesDB'';';
		PRINT '';
		PRINT '  -- Only update stats on a specific table';
		PRINT '  EXEC master.dbo.StatsOptimize';
		PRINT '    @Command = ''OPTIMIZE'', @Databases = ''SalesDB'',';
		PRINT '    @Statistics = ''SalesDB.dbo.Customers'', @Execute = ''Y'';';
		PRINT '';
		PRINT '  -- Update one specific statistic by 4-part name';
		PRINT '  EXEC master.dbo.StatsOptimize';
		PRINT '    @Command = ''OPTIMIZE'', @Databases = ''SalesDB'',';
		PRINT '    @Statistics = ''SalesDB.dbo.Customers.IX_Customers_Email'',';
		PRINT '    @Execute = ''Y'';';
		PRINT '';
		PRINT '=================================================================';
		PRINT 'NOTES:';
...
/*
-- Show help
exec master.dbo.StatsOptimize @Command = 'HELP';

-- Show the usage / sample-size distribution for one database (read-only)
exec master.dbo.StatsOptimize @Command = 'SHOWUSAGE', @Databases = 'SalesDB';

-- Preview statistics that would be updated (read-only, @Execute = 'N')
exec master.dbo.StatsOptimize @Command = 'OPTIMIZE', @Databases = 'SalesDB';
exec master.dbo.StatsOptimize @Command = 'OPTIMIZE', @Databases = 'MarketingDB';

-- Preview across all databases
exec master.dbo.StatsOptimize @Databases = 'ALL_DATABASES';

-- Run for real with defaults
exec master.dbo.StatsOptimize @Databases = 'USER_DATABASES', @Execute = 'Y';

-- Run for real with the TF2371 size curve driving sample size
exec master.dbo.StatsOptimize
	@Command = 'OPTIMIZE',
	@Databases = 'MarketingDB',
	@with_sample_percent = 'TF2371',
	@TimeLimit = 3600, -- 3600 seconds = 1 hr
	@Execute = 'Y';
...
-- Run for real, forcing a fixed 10 percent sample regardless of table size
exec master.dbo.StatsOptimize
	@Command = 'OPTIMIZE',
	@Databases = 'SalesDB',
...
-- Size/staleness/churn only (ignore usage), with a one-hour time budget
exec master.dbo.StatsOptimize
	@Command = 'OPTIMIZE',
...
-- @Statistics: target a specific table (3-part name)
exec master.dbo.StatsOptimize
	@Command = 'OPTIMIZE',
	@Databases = 'SalesDB',
...
-- @Statistics: target a specific statistic (4-part name)
exec master.dbo.StatsOptimize
	@Command = 'OPTIMIZE',
	@Databases = 'SalesDB',
	@Statistics = 'SalesDB.dbo.Customers.IX_Customers_Email',
	@Execute = 'Y';

-- @Statistics: all stats in dbo schema, excluding auto-created _WA stats
exec master.dbo.StatsOptimize
	@Command = 'OPTIMIZE',
	@Databases = 'SalesDB',
	@Statistics = 'SalesDB.dbo.%,-SalesDB.dbo.%._WA%',
	@Execute = 'Y';
*/
